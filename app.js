...
function getFilms() {
    ...
    .then(data => {
        ...
        // Assuming 'movies' is a function that handles the data
    })
    .catch(error => {
        // Handle errors here
...
// Assuming you have an array of movie objects called 'movies'
...
function displayMovies(movies) {
    movies.forEach(movie => {
        ...
        div.innerHTML = movie.title; // Use 'title' to display the movie title
...
function displayMovieDetails(movie) {
    // Implement the logic to display movie details here
    console.log("Clicked on movie with ID: " + movie.id);
}

// Call the function to populate the 'productsElement' with sample movies
document.addEventListener("DOMContentLoaded", function() {
...
function displayMovies(movies) {
    movies.forEach(movie => {
        ...
        card.classList.add("col-md-4"); // Adjust column size as needed
    ...
    const tickets_sold =document.createElement("p");
    tickets_sold.textContent =  "Tickets Sold: " + movie.tickets_sold;
    ...
    btn.addEventListener('submit',()=>{
      if (movie.tickets_sold >= movie.capacity) {
        ...
        // Optionally, you can disable the button as well
        btn.disabled = true;
    } else {
        // Simulate buying a ticket
        movie.tickets_sold++;
        // Update the display to reflect the decreased number of available tickets
        tickets_sold.textContent = "Tickets Sold: " + movie.tickets_sold;
    }
});
